source(paste0("/Users/stellawang/Documents/new_dup/Untitled/DuplicA/app/Scripts/setup.R")) # CHANGE PATH
...
if (parameters$selected_database_protein == "ensembl") {
  parameters$id_type_of_gene_ids <- "ensembl"
} # will convert exon datasets id to this format
...
main_run_workflow <- function(selected_models, parameters) {
  if (parameters$get_public_exon_data) {
    selected_models <- c(selected_models, "exon_datasets")
  }

  # if('Public Datasets' %in% selected_models) {selected_models <- c(selected_models, 'OrthoFinder')}

  # split strings into lists
  if (!is.null(parameters$selected_organisms)) {
    parameters$selected_organisms <- split_into_list(parameters$selected_organisms)
  }
  if (!is.null(parameters$dup_species_list)) {
    parameters$dup_species_list <- split_into_list(parameters$dup_species_list)
  }
  if (!is.null(parameters$tissue_list)) {
    parameters$tissue_list <- split_into_list(parameters$tissue_list)
  }
  ...
  if (!("Public Datasets" %in% selected_models) & ("OrthoFinder" %in% selected_models)) {
    ...
    if ("dnds" %in% selected_models) {
      nuc_output_dir <- parameters$nuc_folder
    }
  }
  ...
  if ("OrthoFinder" %in% selected_models) {
    ...
    if (dir.exists(result_dir)) {
      dir_delete(result_dir)
    } # delete result directory if exists already

    main_OrthoFinder(
      protein_folder = prot_output_dir,
  ...
  out <- main_get_dups_anc_exp_from_OF(
    OF_dir_path = of_output_dir,
  ...
  if ("dnds" %in% selected_models) {
    main_DnDs_output <- main_multispecies_dnds(
      OF_dir_path = of_output_dir,
      dups = dups,
  ...
  if ("expression_shift" %in% selected_models) {
    main_ExpressionShift_output <- main_Expression_Shift(
      OF_dir_path = of_output_dir,
      clean_expression = clean_expression,
  ...
  if ("diversity_divergence" %in% selected_models) {
    main_DiversityDivergence_output <- main_DiversityDivergence(
      OF_dir_path = of_output_dir,
      clean_expression = clean_expression,
      copy_amount = parameters$copy_amount,
      dup_species_list = parameters$dup_species_list,
  ...
  if (("alphafold_db" %in% selected_models) | ("go" %in% selected_models)) {
    ...
    file_organism_table <- data.frame(
      protein_file_name = sort(list.files(prot_output_dir, full.names = T)),
  ...
  if ("postduplication_fates" %in% selected_models) {
    main_postduplication_fates_output <- main_postduplication_fates(
      dups_anc = dups_anc,
      clean_expression = clean_expression,
      v = parameters$v,
      p = parameters$p
  ...
  if ("duplication_mechanism" %in% selected_models) {
    main_dup_mechanism_output <- main_dup_mechanism(
      gn_exons_dir = gn_exons_dir,
      dups_anc = dups_anc,
...
sequential_models <- list(
  "Public Datasets", "OrthoFinder",
  "alphafold_db", "exon_datasets"
)
parallel_models <- list(
  "CDROM", "dnds", "expression_shift", "diversity_divergence", "postduplication_fates", "pathway",
...
main_get_relevant_parameter_list <- function(selected_models, parameters) {
  ...
  if ("Public Datasets" %in% selected_models) {
    ...
    if ("Proteomes" %in% parameters[["data_types"]]) {
      additional_parameter_list <- c(additional_parameter_list, "selected_database_protein")
    }
    if ("CDS" %in% parameters[["data_types"]]) {
      additional_parameter_list <- c(additional_parameter_list, "selected_database_cds")
    }
    if ("Genomes" %in% parameters[["data_types"]]) {
      additional_parameter_list <- c(additional_parameter_list, "selected_database_genome")
    }
    ...
    if ("OrthoFinder" %in% selected_models) {
      ...
      if (parameters[["gene_tree_inference_method"]] == "msa") {
        additional_parameter_list <- c(additional_parameter_list, "msa_method", "tree_method", "msa_trim")
      }
    }
  }
  ...
  if ("dnds" %in% selected_models) {
    if (!"CDS" %in% parameters[["data_types"]]) {
      required_parameter_list <- c(required_parameter_list, "nuc_folder")
    }
  ...
  return(list(
    additional_parameter_list = additional_parameter_list,
