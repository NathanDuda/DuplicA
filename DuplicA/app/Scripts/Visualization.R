...
here_results <- "/Users/stellawang/Documents/new_dup/Untitled/DuplicA/app/Results"

# all_data <- get_all_results(here_results)
# all_data <- all_data %>%
#   mutate(
#     Functional_Category = gsub("pseudo_dup_1", "Pseudo(Dup1)", Functional_Category),
...
generate_figure <- function(data,
  ...
  if (!is.null(color_groups)) {
    data <- data %>% filter(if_any(c(color_groups), ~ !is.na(.)))
  }
  if (!is.null(separate_figure)) {
    data <- data %>% filter(if_any(c(separate_figure), ~ !is.na(.)))
  }
  ...
  if (!is.null(custom_theme)) {
    if (custom_theme == "classic") {
      p <- p + theme_classic()
    }
    if (custom_theme == "bw") {
      p <- p + theme_bw()
    }
    if (custom_theme == "minimal") {
      p <- p + theme_minimal()
    }
    if (custom_theme == "linedraw") {
      p <- p + theme_linedraw()
    }
  }
